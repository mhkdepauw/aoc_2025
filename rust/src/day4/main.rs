...
fn part2(megavec: &Vec<String>) -> i32 {
    let mut imegavec: Vec<Vec<char>> = megavec
        .clone()
        .iter()
        .map(|s| s.chars().collect())
        .collect();
        ...
        for j in 0..imegavec[i].len() {
            if imegavec[i][j] == '@' {
                ...
                if inotfirst {
                    ...
                    if jnotfirst && imegavec[i - 1][j - 1] == '@' {
                        count += 1
                    }
                    if jnotlast && imegavec[i - 1][j + 1] == '@' {
                        count += 1
                    }
                };
                if jnotfirst && imegavec[i][j - 1] == '@' {
                    count += 1
                }
                if jnotlast && imegavec[i][j + 1] == '@' {
                    count += 1
                }
                if inotlast {
                    if imegavec[i + 1][j] == '@' {
