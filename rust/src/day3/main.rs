...
fn highest_substring(megavec: &Vec<String>, string_length: usize) -> u64 {
    ...
    for numvec in megavec.iter().map(|s| {
        ...
        for l in 0..string_length {
            let sliced_numvec: Vec<u32> =
                numvec[start_i..numvec.len() - ((string_length - 1) - l)].to_owned();
